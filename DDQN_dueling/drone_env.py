...
class drone_env:
    ...
    def moveByDist(self,diff, forward = False):
        ...
        self.client.moveByVelocity(diff[0], diff[1], diff[2], 0.5 ,drivetrain = AirSimClient.DrivetrainType.ForwardOnly, yaw_mode = temp)
        time.sleep(0.5)
...
class drone_env_gridworld(drone_env):
    ...
    def step(self,action):
        ...
        if self.client.getCollisionInfo().has_collided:
            ...
            info = "collision"
        if (distance(pos_,self.aim)>100):
            reward = -100
            done = True
            info = "out of range"
        if self.step_taken > self.maximum_step:
            reward = -100
            done = True
            info = "time out"
