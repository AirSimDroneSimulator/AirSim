...
class Critic(nn.Module):
	...
	def __init__(self, state_dim, action_dim):
		...
		self.fcs1 = nn.Linear(self.val_state_dim,256)
		self.fcs1.weight.data = fanin_init(self.fcs1.weight.data.size())
		self.fcs2 = nn.Linear(256,128)
		self.fcs2.weight.data = fanin_init(self.fcs2.weight.data.size())

		self.conv1 = nn.Sequential(         # input shape (1, 56, 56)
			nn.Conv2d(1,32,5,1,2),          # output shape (32, 56, 56),kernel_size = 5, stride = 1, padding = 2
			nn.ReLU(),                      # activation
	...
	def forward(self, state, action):
		...
		s2 = F.relu(self.fcs2(s1))   #128
		c1 = self.conv1(img)
		c2 = self.conv2(c1)
		c3 = self.conv3(c2)
		c4 = self.conv4(c3)
		c4 = torch.squeeze(c4) # 128
		c4 = c4.view(size,-1)
		sout = torch.cat((c4,s2),dim = 1) #256
